...
//  Snacktacular
//
//  Created by Allen Li on 11/2/19.
    ...
    func updateAverageRating(completed: @escaping ()->()) {
        ...
        reviewsRef.getDocuments { (querySnapshot, error) in
            guard error == nil else {
                print("*** ERROR: failed to get query snapshot of reviews for reviewsRef: \(reviewsRef.path)")
                return completed()
            }
            var ratingTotal = 0.0
            ...
            spotRef.setData(dataToSave) { error in
                guard error == nil else {
                    print("*** ERROR: updating document \(self.documentID) in spot after changing averageReview and numberOfReviews")
                    return completed()
                }
                completed()
            }
